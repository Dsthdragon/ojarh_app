...
if(is_uploaded_file($user->file_tmp)){
    if($user->file_size > 1097152 ){
    	...
    	header('Location: ../../buyer/profile_setting.php?result=File size must not be more than 1MB.');
        return false;
    }else{
    	if($user->updateprofilepic()){
    		if ( !is_dir( "../../seller/profilepicture/".$user->userid ) ) {
				...
				if(move_uploaded_file($user->file_tmp, $targetPath)) {
					...
					header('Location: ../../buyer/profile_setting.php?result=Profile picture updated successfully.');
					return true;
				}
    		}else{
    			$targetPath = "../../seller/profilepicture/".$user->userid.'/'.$user->file_name;
			...
			header('Location: ../../buyer/profile_setting.php?result=could not update profile picture.');
			return false;
		}
    }
	...
	header('Location: ../../buyer/profile_setting.php?result=Please select a file.');
	return false;
}
